...
func initHbScheduler(conf *config.EventConfiguration, defaultInterval time.Duration, namingCodes map[string]string, state ha.AgentState) *scheduler.Scheduler {
...
func initNfcNamingCode(nfcNamingCodes []config.NfcNamingCode) map[string]string {
...
// It initializes the AlertReceiver server to receive and handle Alert event from prometheus.
func (agent *Agent) StartAgent(bind string, ves evel.VESCollectorIf) {
	agent.listen(bind, ves)
	agent.serve(ves)
}

func (agent *Agent) listen(bind string, ves evel.VESCollectorIf) {
...
func (agent *Agent) serve(ves evel.VESCollectorIf) {
	for {
...
func (agent *Agent) leaderStep(ves evel.VESCollectorIf) bool {
	// Demultiplex events
	select {
...
func (agent *Agent) handleAlertReceived(ves evel.VESCollectorIf, messageFault rest.MessageFault) {
...
func (agent *Agent) triggerMeasurementEvent(ves evel.VESCollectorIf) {
...
func (agent *Agent) triggerHeatbeatEvent(ves evel.VESCollectorIf) {
	triggerScheduler(agent.hbSched, &agent.hbTimer, func(res interface{}) error {
		return ves.PostEvent(res.(evel.Event))
	})
}
