...
func (ves *ClusterMock) PostEvent(evt evel.Event) error {
	args := ves.MethodCalled("PostEvent", evt)
	return args.Error(0)
}

func (ves *ClusterMock) PostBatch(batch evel.Batch) error {
...
type AgentTestSuite struct {
	...
	eventConf   *config.EventConfiguration
...
func (suite *AgentTestSuite) SetupSuite() {
	...
	eventConf := config.EventConfiguration{
		MaxSize:        200,
		NfNamingCode:   "hspx",
		NfcNamingCodes: []config.NfcNamingCode{oam, etl},
	}
	measConf := config.MeasurementConfiguration{
		DefaultInterval: 2 * time.Second,
	}
	collectorConf := config.CollectorConfiguration{
		FQDN:     "localhost",
		Port:     1234,
		Topic:    "mytopic",
		User:     "myuser",
	...
	backupConf := config.CollectorConfiguration{
		FQDN:     "",
		Port:     0,
		Topic:    "",
		User:     "",
...
func (suite *AgentTestSuite) TestAgent() {
	...
	suite.cluster.On("PostEvent", mock.AnythingOfType("*evel.EventFault")).Once().Return(nil)
	// PostEvent with heartbeat MUST be called at least once
	suite.cluster.On("PostEvent", mock.AnythingOfType("*evel.HeartbeatEvent")).Once().Return(nil)
	// But it sometimes may be called a second time, depending on timing.
	suite.cluster.On("PostEvent", mock.AnythingOfType("*evel.HeartbeatEvent")).Maybe().Return(nil)
