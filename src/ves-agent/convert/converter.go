...
var severityToPriority = map[evel.Severity]evel.EventPriority{
	evel.SeverityCritical: evel.PriorityHigh,
	evel.SeverityMajor:    evel.PriorityMedium,
	evel.SeverityMinor:    evel.PriorityLow,
	evel.SeverityWarning:  evel.PriorityLow,
	evel.SeverityNormal:   evel.PriorityNormal,
}
...
func AlertToFault(alert template.Alert, fm *FaultManager, namingCodes map[string]string) (StatusResult, *evel.EventFault, CommitFunc) {
	var storeStatus StatusResult
	var alertName string
	var specificProblem string
	//var eventFault *evel.EventFault
	var id int32

	label := alert.Labels
	annotations := alert.Annotations
	log.Debugln("convert alert to VES event fault: ", label["alertname"])
	severity := evel.Severity(strings.ToUpper(label["severity"]))
...
func buildAdditionalInfos(service string, aaiMapping string) []evel.EventField {
	...
	if len(alaInfos) == 0 || len(alaInfos) != len(eventNames)+1 {
		...
		for i := 1; i < len(alaInfos); i++ {
			events = append(events, evel.EventField{Name: eventNames[i-1], Value: alaInfos[i]})
		}
	}
	return events
}
