...
type FaultManager struct {
	...
	lock  *sync.Mutex
	conf  *config.EventConfiguration
}
...
func NewFaultManagerWithState(conf *config.EventConfiguration, state FaultManagerState) *FaultManager {
	return &FaultManager{
		//index:   0,
...
func NewFaultManager(conf *config.EventConfiguration) *FaultManager {
...
func (fm *FaultManager) GetEventConf() *config.EventConfiguration {
	return fm.conf
}
