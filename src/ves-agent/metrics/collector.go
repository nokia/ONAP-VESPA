...
type Collector struct {
	...
	evtCfg      *config.EventConfiguration    // Generals event configuration
...
func NewCollectorWithState(cfg *config.MeasurementConfiguration, evtCfg *config.EventConfiguration, namingCodes map[string]string, state CollectorState) (*Collector, error) {
...
func NewCollector(cfg *config.MeasurementConfiguration, evtCfg *config.EventConfiguration, namingCodes map[string]string) (*Collector, error) {
...
func (col *Collector) CollectMetrics(from, to time.Time, interval time.Duration) (EventMeasurementSet, error) {
	...
	metrics := NewEventMeasurementSetBuilder(MeasurementFactoryFunc(func(vmID string, timestamp time.Time) (*evel.EventMeasurements, error) {
		if vmID == "" {
			vmID = VNFName
		}
		...
		meas := evel.NewMeasurements(evtName, fmt.Sprintf("Measurements%.10d", id), vmID, interval, timestamp.Add(-interval), timestamp)
