...
// of `evel.Key`
func (keys MeasKeys) JSONObjectKeys() []evel.Key {
	jKeys := []evel.Key{}
	for k, v := range keys {
		s := v // Copy string before getting a reference to it
		jKeys = append(jKeys, evel.Key{KeyName: k, KeyValue: &s})
	}
	return jKeys
}

// MatchJSONObjectKeys checks if this set of key/value pairs match exactly  with
// the given list of `evel.Key`
func (keys MeasKeys) MatchJSONObjectKeys(jKeys []evel.Key) bool {
...
type EventMeasurementSet []*evel.EventMeasurements

// Batch converts the measurement set into a batch of events
func (set EventMeasurementSet) Batch() evel.Batch {
	batch := make([]evel.Event, len(set))
	for i, evt := range set {
		batch[i] = evt
	}
	return batch
...
type MeasurementFactory interface {
	Create(vmID string, timestamp time.Time) (*evel.EventMeasurements, error)
}

// MeasurementFactoryFunc is a function which can be used as a MeasurementFactory
type MeasurementFactoryFunc func(vmID string, timestamp time.Time) (*evel.EventMeasurements, error)

// Create is called to create a new `evel.EventMeasurements`
func (f MeasurementFactoryFunc) Create(vmID string, timestamp time.Time) (*evel.EventMeasurements, error) {
...
type EventMeasurementSetBuilder struct {
	...
	provider MeasurementFactory  // A provider to create new evel.EventMeasurements
...
func (bld *EventMeasurementSetBuilder) find(vmID string, timestamp time.Time) (*evel.EventMeasurements, error) {
...
func (bld *EventMeasurementSetBuilder) Set(fields string, vmID string, timestamp time.Time, value float64, keys MeasKeys) error {
	...
	// Get or create the evel.EventMeasurements
...
func (bld *EventMeasurementSetBuilder) SetAdditionalObject(vmID, objectName, objectInstance string, timestamp time.Time, value float64, keys MeasKeys) error {
	...
	var additionalObject *evel.JSONObject
	...
	if additionalObject == nil {
		metric.AdditionalObjects = append(metric.AdditionalObjects, evel.JSONObject{ObjectName: objectName})
		additionalObject = &metric.AdditionalObjects[len(metric.AdditionalObjects)-1]
	}
	var instance *evel.JSONObjectInstance
